...
export default {
	...
	computed: {
		...
		exportFilePrefix() {
			return SOURCE_EXPORT_PREFIX[this.source] || '运营日志';
		}
	},
	mounted() {
		this.search();
	...
	methods: {
		...
		mapExportRow(item) {
			if (this.source === 'push') {
				return {
					接收时间: this.fmtTs(item.time),
					类型: item.pushType || '',
					成功: item.success ? '是' : '否',
					代理商: item.firstagentid || '',
					摘要: item.summary || '',
					失败原因: item.errorMsg || '',
					记录ID: item._id || ''
				};
			}
			if (this.source === 'robot') {
				return {
					时间: this.fmtTs(item.time),
					通道: item.channel || '',
					成功: item.success ? '是' : '否',
					Webhook: item.webhookMasked || '',
					内容摘要: item.contentPreview || '',
					错误: item.errmsg || '',
					记录ID: item._id || ''
				};
			}
			return {
				...
				模块: item.module || '',
				操作: item.action || '',
				目标: item.targetName || item.targetId || '',
				内容: item.content || '',
				IP: item.ip || '',
				来源: item.operatorSource || '',
				记录ID: item._id || ''
			};
		},
		exportData(type) {
