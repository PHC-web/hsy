	...
	<view class="fix-top-window">
		...
		<uni-popup ref="auditPopup" type="center">
			<view class="audit-panel">
				<text class="audit-title">{{ batchMode ? '流水优化批量审核' : '流水优化审核' }}</text>
				<text class="audit-hint">
					{{
						batchMode
							? `已选 ${selectedPendingIds.length} 笔待审`
							: `机具/单号：${auditRow ? auditRow.snTradeDisplay : '-'}`
					}}
				</text>
				<textarea v-model="auditRemark" class="audit-textarea" placeholder="请填写审核意见（必填）" />
				<view class="audit-actions">
					...
					<button
						v-if="batchMode"
						class="uni-button"
						:type="batchStatus === 'rejected' ? 'warn' : 'primary'"
						size="mini"
						@click="submitBatchAudit"
					>
						{{ batchStatus === 'rejected' ? '确认驳回' : '确认通过' }}
					</button>
				</view>
			</view>
		</uni-popup>
...
export default {
	...
	computed: {
		selectedPendingIds() {
			return Object.keys(this.selectedMap || {}).filter((k) => this.selectedMap[k]);
		},
		allPendingChecked() {
			const pending = (this.list || []).filter((x) => x && x.status === 'pending' && x.id);
			if (!pending.length) return false;
			return pending.every((x) => !!this.selectedMap[x.id]);
		}
	},
	mounted() {
		this.search();
	...
	methods: {
		...
		toggleSelect(item) {
			if (!item || !item.id || item.status !== 'pending') return;
			this.selectedMap = { ...this.selectedMap, [item.id]: !this.selectedMap[item.id] };
		},
		toggleSelectAllPending() {
			const pending = (this.list || []).filter((x) => x && x.status === 'pending' && x.id);
			const allOn = pending.length && pending.every((x) => !!this.selectedMap[x.id]);
			const next = { ...this.selectedMap };
			for (const x of pending) {
				if (allOn) delete next[x.id];
				else next[x.id] = true;
			}
		...
		openBatchAudit(status) {
			if (!this.selectedPendingIds.length) {
				uni.showToast({ title: '请先勾选待审流水', icon: 'none' });
				return;
			}
			this.batchMode = true;
			this.batchStatus = status === 'rejected' ? 'rejected' : 'approved';
			this.auditRow = null;
			this.auditRemark = '';
			this.$refs.auditPopup.open();
		},
		...
		submitBatchAudit() {
			...
			uni.showLoading({ title: '批量提交中...', mask: true });
			this.$request(
				'flowOptAuditTradeBatch',
				{ tradeIds: ids, status: this.batchStatus, remark },
				{ functionName: 'machine' }
			)
				.then((res) => {
					if (res.code === 0) {
						uni.showToast({ title: res.message || '已处理', icon: 'success' });
						this.selectedMap = {};
						this.closeAudit();
						this.search();
					} else {
				...
				.then((res) => {
					...
					if (res.code === 0) {
						...
						for (const id of Object.keys(this.selectedMap || {})) {
							if (idSet.has(id) && this.selectedMap[id]) next[id] = true;
						}
						this.selectedMap = next;
					} else {
		...
		toXml(rows) {
			const esc = (s) =>
				String(s == null ? '' : s)
					...
					.replace(/>/g, '&gt;');
			const items = rows
				.map((r) => `<item>${Object.entries(r).map(([k, v]) => `<${k}>${esc(v)}</${k}>`).join('')}</item>`)
				.join('');
			return `<?xml version="1.0" encoding="UTF-8"?><flowOpts>${items}</flowOpts>`;
		},
		toHtmlTable(rows) {
			const keys = Object.keys(rows[0] || {});
		...
		async exportData(type) {
			try {
				...
				const ts = Date.now();
				if (type === 'json')
					this.downloadFile(`优化管理_${ts}.json`, JSON.stringify(rows, null, 2), 'application/json;charset=utf-8');
				else if (type === 'xml') this.downloadFile(`优化管理_${ts}.xml`, this.toXml(rows), 'application/xml;charset=utf-8');
				else if (type === 'csv') this.downloadFile(`优化管理_${ts}.csv`, this.toCsv(rows), 'text/csv;charset=utf-8');
				else if (type === 'txt') this.downloadFile(`优化管理_${ts}.txt`, this.toTxt(rows), 'text/plain;charset=utf-8');
				else if (type === 'word') this.downloadFile(`优化管理_${ts}.doc`, this.toHtmlTable(rows), 'application/msword');
				else if (type === 'excel')
					this.downloadFile(`优化管理_${ts}.xls`, this.toHtmlTable(rows), 'application/vnd.ms-excel');
			} catch (e) {
